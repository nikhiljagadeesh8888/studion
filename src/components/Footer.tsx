...
const Footer = () => {
  ...
  // ✅ Fetch logo from Strapi (same as Header)
  useEffect(() => {
    const fetchLogo = async () => {
      try {
        const res = await fetch(`${API}/api/logos?populate=*`);
    ...
    <footer className="px-6 md:px-12 lg:px-20">
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-[1fr_1px_1fr] gap-8 md:gap-12 py-16 md:py-24">
        <div>
          ...
          <p className="body-text text-sm text-muted-foreground max-w-md">
            Studio N's style is editorial with a relaxed documentary feel.
            Specialising in portrait, wedding, product and model photography
            across the UK and destination shoots worldwide.
          </p>
        </div>
        ...
        <div className="md:pl-8">
          <h4 className="font-serif text-foreground text-lg uppercase tracking-[0.15em] mb-6">
            Recent Work
          </h4>
          <ul className="space-y-3">
            <li>Golden Hour Wedding at the Manor</li>
            <li>Editorial Bridal Portrait Session</li>
            <li>Fashion Model Shoot in Leicester</li>
          </ul>
        </div>
      </div>

      {/* ✅ LOGO DIVIDER (DYNAMIC) */}
      <div className="max-w-6xl mx-auto flex items-center gap-6 py-4">
        <div className="flex-1 h-[1px] bg-border" />
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-[1fr_1px_1fr] gap-8 md:gap-12 py-16 md:py-24">
        <div className="flex flex-col gap-6">
          <h4 className="font-serif text-foreground text-lg uppercase tracking-[0.15em]">
            Follow Along
          </h4>
          <div className="flex gap-6">
            <a
              href="https://www.instagram.com/studio.__n"
              target="_blank"
              rel="noopener noreferrer"
              className="underline"
            >
